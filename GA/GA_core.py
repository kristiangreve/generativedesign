...
def dir_crowding(population):
    ...
    for pareto_list in pareto_dict.values():
        #print('Pareto front', pareto_list[0].pareto, 'with size: ', len(pareto_list))
        #print('...and adjacency score: ', pareto_list[0].adjacency_score)
...
def breeding(population):
    ...
    while len(children) < len(population):
        ...
        if parent1 != parent2:
            child1,child2 = crossover(parent1,parent2) #Crossing dir lists
...
def selection(pop_size, population):
    ...
    for pareto_counter in range(1,len(population)):
        ...
        if (len(new_gen)+len(pareto_dict[pareto_counter])) < pop_size:
            ...
            sorted_pareto = sorted(pareto_dict[pareto_counter], key=lambda x: x.dir_score, reverse=False)
